...
bool GameImpl::init(AbstractSoundManager& sound_manager,
  ...
  if (player_.stop_tick == 0)
  {
    for (auto&& enemy : level_->enemies)
    {
      if (geometry::isColliding(prect, geometry::Rectangle(enemy->position, enemy->size)))
      {
        ...
        {
          enemy->on_hit(prect, *sound_manager_, prect, *level_, true);
        }
      }
    }
...
void GameImpl::update_player(const PlayerInput& player_input)
{
  ...
  {
    level_->reverse_gravity();
    sound_manager_->play_sound(SoundType::SOUND_SWITCH);
    LOG_DEBUG("Reverse gravity %s", level_->gravity < 0 ? "ON" : "OFF");
  }
